...
pub fn exported_names(source: &str) -> Vec<String> {
    let mut lowerer = lower::Lowerer::new();
    let tokens = crate::lexer::Lexer::new(source).lex();
    let file_id = lowerer.add_file("scan.mond".into(), source.into());
...
/// Extract the modules that a lib.mond publicly re-exports via `(pub use X)`.
/// Used to gate `(use std/io)` — io must be pub-used by lib.mond.
...
/// The erlang_fn_name matches what codegen emits: `mond_test_0`, `mond_test_1`, ...
